...
class MeetResults:
    ...
    class RelayIdEntry:
        ...
        def __str__(self) -> str:
            return f"{self.swimmer_names} - {self.club}"
    ...
    class EventResult:
        ...
        def __str__(self) -> str:
            return f"{self.event_round} {self.gender} {self.event_name} {self.age_group}: {str(self.rankings)}"
    ...
    def __extract_results_from_event(self, event_node: ET.Element):
        ...
        for node in event_node:
            if node.tag == "SWIMSTYLE":
                ...
                event_name = f"{relay_prepend}{node.attrib.get("distance")} {node.attrib.get("stroke")}"
    ...
    def __get_ranking_entry_for_result_id_relay(self, result_id: str, placing: int) -> RankingsEntry:
    ...
    def construct_rankings(self, filters: list[str]):
        ...
        for result in self.meet_results:
            event_name = f"{result.event_round} {result.gender} {result.event_name} {result.age_group}"
            
            if "ONLY_FINALS" in rest_filters and result.event_round == "PRE":
                continue

            # DNS, DQ, DNF -> placed at the end so doesn't matter that they are included
            placing = 1
            for result_id in result.rankings:
    ...
    def print_rankings(self):
        ...
        for event in self.results:
            if self.results[event] == []:
                continue
            
            self.log.info(f"Rankings for {event}:")
            for entry in self.results[event]:
                self.log.info(f"{entry.placing}: {entry.swimmer_name} - {entry.club} - {entry.swim_time}")
            self.log.info(" ")
        for event in self.results_relays:
            if self.results_relays == []:
                continue
            
            self.log.info(f"Results for {event}")
            for entry in self.results_relays[event]:
                self.log.info(f"{entry.placing}: {entry.swimmer_name} - {entry.club} - {entry.swim_time}")
            self.log.info(" ")
    
    def __str__(self) -> str:
        return str(self.club_results)